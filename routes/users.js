...
router.get("/nums", authenticateToken, async (req, res) => {
  let expertsNum = await User.find({ isExpert: true }).exec();
  let usersNum = await User.find({}).exec();

  res.send({ expertsNum: expertsNum.length, usersNum: usersNum.length });
});
...
router.put("/me", authenticateToken, async (req, res) => {
  ...
  const updatedUser = await User.findOneAndUpdate({ _id }, userDetails, {
    omitUndefined: true,
    runValidators: true,
    new: true,
  }).exec();
  console.log("the update of the user", updatedUser);
